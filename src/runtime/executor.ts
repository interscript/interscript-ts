...
const BUILTIN_FUNCTIONS: Record<
  string,
  (input: string, opts?: Record<string, unknown>) => string
> = {
  downcase,
  upcase,
  ...
  parallel: (rule, ctx) => {
    ...
    for (const inner of rule.rules) {
      ...
      const hasConstraints =
        inner.before || inner.after || inner.notBefore || inner.notAfter;
...
function executeSubRule(rule: SubRule, ctx: ExecutionContext): void {
  ...
  try {
    ...
    throw new MapLogicError(
      `Invalid pattern compiled from rule: ${patternParts.join("")}`,
      {
        cause: e,
      },
    );
